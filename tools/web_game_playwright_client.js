...
const debugLogPath = process.env.WEB_GAME_DEBUG_LOG || "";
...
async function main() {
  ...
  debugLog(`browser:launch headless=${args.headless}`);
  const launchOptions = {
  ...
  debugLog(`page:goto ${args.url}`);
  await page.goto(args.url, { waitUntil: "domcontentloaded" });
  debugLog("page:goto-complete");
  await page.waitForTimeout(500);
  debugLog("page:post-timeout");
  await page.evaluate(() => {
    window.dispatchEvent(new Event("resize"));
  });
  debugLog("page:resize-dispatched");

  let canvas = await getCanvasHandle(page);
  debugLog(`canvas:found=${Boolean(canvas)}`);
  ...
  for (let i = 0; i < args.iterations; i++) {
    debugLog(`iteration:${i}:start`);
    if (!canvas) canvas = await getCanvasHandle(page);
    debugLog(`iteration:${i}:canvas=${Boolean(canvas)}`);
    await doChoreography(page, canvas, steps);
    debugLog(`iteration:${i}:choreography-complete`);
    await sleep(args.pauseMs);
    debugLog(`iteration:${i}:pause-complete`);

    const shotPath = path.join(args.screenshotDir, `shot-${i}.png`);
    await captureScreenshot(page, canvas, shotPath);
    debugLog(`iteration:${i}:screenshot=${shotPath}`);
    ...
    debugLog(`iteration:${i}:state=${Boolean(text)}`);
    if (text) {
    ...
    if (freshErrors.length) {
      ...
      debugLog(`iteration:${i}:errors=${freshErrors.length}`);
      break;
    }
  }
